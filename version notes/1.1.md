...
- we can get all available labeled protein names from the database: `protein.get_all_labeled_protein_names(protein_records)`


- we can get protein similiarities (mutations) by name, since this can save us a lot of time instead of processing all protein mutations `protein.get_all_labeled_protein_names(protein_records)`
...
# Assessing Mutations with the Blosum Matrix Mutations
...
Here we will only determine different amino acid substitions. If we were substiting a similar amino acid we would have analyze the nucleotide sequence to determine synonymous codons, something we can do later.
